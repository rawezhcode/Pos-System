...
class ProductCart extends Component
{
    ...
    public function ItemPlus($RowId)
    {
        $cart = Cart::instance($this->cart_instance);
        $cart_item = $cart->get($RowId);
        $cart->update($RowId, ['qty' => $cart_item->qty + 1]);
    }

    public function ItemMinus($RowId)
    {
        $cart = Cart::instance($this->cart_instance);
        $cart_item = $cart->get($RowId);
        $cart->update($RowId, ['qty' => $cart_item->qty - 1]);
    ...
    public function productSelected($product) {
        ...
        if ($exists->isNotEmpty()) {
            $cart_item = $cart->get($this->getRowId);
            $cart->update($this->getRowId, ['qty' => $cart_item->qty + 1]);
